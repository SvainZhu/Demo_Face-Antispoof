...
def train_classifier_mat(data_dir):
    ...
    # 直接读取mat文件
    LBPDCT_train = loadmat(data_dir + '/RE_LBPDCT_I72_S3_P1_train.mat')['LBPDCT_train'][0:90]
    data = list(itertools.chain(*LBPDCT_train))

    train_label_vector = loadmat(data_dir + '/label_RE_train.mat')['label_RE_train']
...
class Ui_MainWindow(object):
    ...
    def __init__(self, MainWindow):
        ...
        self.model = train_classifier_mat('LBP_DCT/RE')
    ...
    def takePhoto(self):
        if self.timer_camera.isActive():
            ...
            test_dir = os.path.join('./demo', 'LBP_DCT')
            class_name = os.path.join(test_dir, 'photos')
            if not os.path.exists(class_name):
                os.makedirs(class_name)
            face_dir = os.path.join(test_dir, 'faces')
